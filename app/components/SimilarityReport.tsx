...
const SimilarityReport: React.FC<SimilarityReportProps> = ({ 
  ...
  React.useEffect(() => {
    if (similarityData && onSimilarityValidation) {
      const hasDangerousSimilarity = results.some(
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      ...
      <div className="mb-4">
        <div className="flex justify-between mb-1">
          <span className="text-sm font-medium">Similarity level</span>
        ...
        <p className={`mt-2 text-sm text-${status.color}-600`}>{status.message}</p>
      </div>
      
      {results.length > 0 && (
        <div>
          <h4 className="font-medium text-gray-700 mb-2">Documents Similaires Trouvés:</h4>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`px-2 py-1 rounded text-sm ${
                      item.similarity >= status.similarity_danger_threshold ? 'bg-red-600 text-white' : 
                      item.similarity >= status.similarity_warning_threshold ? 'bg-orange-100 text-orange-800' : 
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-red-600 text-sm mt-1">
                            Le taux de similarité ({item.similarity.toFixed(1)}%) dépasse le seuil autorisé ({status.similarity_danger_threshold}%).
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          {results.some(item => item.similarity >= status.similarity_danger_threshold) && (
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-red-700 mt-2">
                    Votre document présente un taux de similarité trop élevé avec des documents existants.
                    Pour pouvoir soumettre votre mémoire, vous devez :
                  </p>
                  <ul className="list-disc list-inside text-red-700 mt-2 space-y-1">
                    <li>Réviser le contenu de votre document</li>
                    <li>Réduire les similarités en dessous de {status.similarity_danger_threshold}%</li>
